...
export default function EditProject({
  ...
  const { data: initialProject, error } = useSWR(
  ...
  // Initialize project state when data is loaded
  if (initialProject && !project) {
    setProject({
      ...initialProject,
      serviceArea: initialProject.serviceArea || [],
      challengeImages: initialProject.challengeImages || [],
      outcomes: initialProject.outcomes || [],
      testimonial: initialProject.testimonial || {
        author: "",
        position: "",
        quote: "",
        image: "",
  ...
  const removeTag = (index: number) => {
    setProject({
      ...project,
      tags: project.tags.filter((_: any, i: number) => i !== index),
    });
  };
  ...
  const removeServiceArea = (index: number) => {
    setProject({
      ...project,
      serviceArea: project.serviceArea.filter(
  ...
  const removeChallengeImage = (index: number) => {
    setProject({
      ...project,
      challengeImages: project.challengeImages.filter(
        (_: any, i: number) => i !== index
      ),
    });
  };
  ...
  const removeOutcome = (index: number) => {
    setProject({
      ...project,
      outcomes: project.outcomes.filter((_: any, i: number) => i !== index),
    });
  };
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {project.serviceArea.map((area: string, index: number) => (
                  <Badge
                    key={index}
                    variant="secondary"
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                {project.challengeImages.map((image: string, index: number) => (
                  <div
                    key={index}
